        ...
        >
          ...
          <n-form-item class="default-color remember-forgot">
            <div class="flex-between-wrapper">
              <div class="left">
                <n-checkbox v-model:checked="autoLogin">30天内免登录</n-checkbox>
              </div>
              <div class="right">
...
<script lang="ts" setup>
  ...
  import { ACCESS_TOKEN, REFRESH_TOKEN, AUTO_LOGIN } from '@/store/mutation-types';
  ...
  const onVCodeSuccess = async () => {
    ...
    try {
      ...
      if (resultCode.status === 'success') {
        ...
        const autoLingCheck = autoLogin.value;

        const result = await userStore.login(params, autoLingCheck);
  ...
  onMounted(() => {
    if (
      storage.exists(AUTO_LOGIN) &&
      storage.exists(ACCESS_TOKEN) &&
      storage.exists(REFRESH_TOKEN)
    ) {
      const userAutoLogin = storage.get(AUTO_LOGIN, false);
      const accessToken = storage.get(ACCESS_TOKEN);
      const refreshToken = storage.get(REFRESH_TOKEN);
      console.log('userAutoLogin:', userAutoLogin);
