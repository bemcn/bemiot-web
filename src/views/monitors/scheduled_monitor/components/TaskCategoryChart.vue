<template>
  <n-card :bordered="false" title="任务分类执行统计" class="monitor-card" size="small">
...
<script lang="ts" setup>
  ...
  // 任务分类
  const categories = ['数据收发', '场景任务', '桥接数据', '系统监测', '消息通知'];
  ...
  const initChartData = () => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // 为每个分类生成数据
      seriesData[0].data.push(Math.floor(Math.random() * 100)); // 数据收发
      seriesData[1].data.push(Math.floor(Math.random() * 80)); // 场景任务
      seriesData[2].data.push(Math.floor(Math.random() * 60)); // 桥接数据
      seriesData[3].data.push(Math.floor(Math.random() * 70)); // 系统监测
      seriesData[4].data.push(Math.floor(Math.random() * 90)); // 消息通知
    }
