<template>
  <n-card :bordered="false" title="故障异常历史统计" class="monitor-card" size="small">
...
<script lang="ts" setup>
  ...
  const initChartData = () => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // 为每个分类生成数据
      seriesData[0].data.push(Math.floor(Math.random() * 10)); // 定时任务
      seriesData[1].data.push(Math.floor(Math.random() * 8)); // 场景联动
      seriesData[2].data.push(Math.floor(Math.random() * 6)); // MQTTDrivice
      seriesData[3].data.push(Math.floor(Math.random() * 7)); // ModBus Drivice
      seriesData[4].data.push(Math.floor(Math.random() * 9)); // TCP Drivice
      seriesData[5].data.push(Math.floor(Math.random() * 5)); // Video Server
    }
