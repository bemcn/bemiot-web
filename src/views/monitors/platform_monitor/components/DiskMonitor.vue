  ...
  <n-card :bordered="false" title="磁盘监控" class="monitor-card" size="small">
    <div class="monitor-info">
      <n-descriptions label-placement="left" size="small" :column="2">
        <n-descriptions-item label="总容量"> 1 TB </n-descriptions-item>
        <n-descriptions-item label="已使用"> 320 GB </n-descriptions-item>
        <n-descriptions-item label="使用率">
          <n-progress type="line" :percentage="diskUsage" :color="getColor(diskUsage)" />
        </n-descriptions-item>
      </n-descriptions>
    </div>
...
<script lang="ts" setup>
  ...
  const initChartData = () => {
    ...
    for (let i = 0; i < 60; i++) {
      timeData.push(`${i}s`);
      usageData.push(Math.floor(Math.random() * 10) + 30);
    }
  };
  ...
  const updateData = () => {
    ...
    // 添加新数据点
    const newData = Math.floor(Math.random() * 15) + 30;
    usageData.shift();
