<template>
  <n-card :bordered="false" title="缓存分类使用统计" class="monitor-card" size="small">
...
<script lang="ts" setup>
  ...
  // 缓存分类
  const categories = ['系统数据', 'IOT数据', '业务数据', '其他'];
  ...
  const initChartData = () => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // 为每个分类生成0.1到20480之间的随机数作为缓存使用量(MB)
      seriesData[0].data.push(Math.random() * (20480 - 0.1) + 0.1); // 系统数据
      seriesData[1].data.push(Math.random() * (20480 - 0.1) + 0.1); // IOT数据
      seriesData[2].data.push(Math.random() * (20480 - 0.1) + 0.1); // 业务数据
      seriesData[3].data.push(Math.random() * (20480 - 0.1) + 0.1); // 其他
    }
