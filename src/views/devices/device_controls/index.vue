...
<script lang="ts" setup>
  ...
  import {
    getDeviceControlsPageList,
    delDeviceControlsById,
    delDeviceControlsByIds,
    DeviceControlsPageParams,
  } from '@/api/devices/deviceControls';
  ...
  const auth = userStore.parseAuthByModule('device_controls');
  ...
  const loadDataTable = async (res: any) => {
    ...
    let params = {} as DeviceControlsPageParams;
    ...
    const result = (await getDeviceControlsPageList(params)) as unknown as {
      status: string;
      message: string;
      data: PageProtocols;
    };
  ...
  const handleEdit = (record: Recordable) => {
    const controlRuleData = JSON.parse(record.controlRule);

    formParams.value = {
      controlId: record.controlId,
      controlName: record.controlName,
      controlFunc: record.controlFunc,
      delaySecond: record.delaySecond,
      controlRule: record.controlRule,
      controlRuleData,
      remark: record.remark,
    };
    modalTitle.value = '编辑设备群控';
    action.value = 'edit';
    showModal.value = true;
  };
  ...
  const handleDel = async (record: Recordable) => {
    ...
    try {
      const result = (await delDeviceControlsById(params)) as unknown as {
        status: string;
        message: string;
      };
  ...
  const handleDelArray = async () => {
    ...
    const result = (await delDeviceControlsByIds(params)) as unknown as {
      status: string;
      message: string;
    };
    if (result.status === 'success') {
