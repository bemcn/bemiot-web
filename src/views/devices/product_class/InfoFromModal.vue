<template>
  ...
  >
    <n-form
      ...
      class="py-4"
    >
      <n-form-item label="分类名称" path="className">
        <n-input
          maxlength="20"
          placeholder="请输入分类名称"
          v-model:value="localParams.className"
          clearable
        />
      </n-form-item>
      <n-form-item label="上级分类" path="levelId">
        <n-select
...
<script lang="ts" setup>
  ...
  import { addProductClass, editProductClass } from '@/api/devices/productClass';
  ...
  const confirmForm = (e: any) => {
    ...
    formRef.value.validate(async (errors: any) => {
      if (!errors) {
        ...
        if (props.action === 'add') {
          result = (await addProductClass(localParams.value)) as unknown as {
            status: string;
            message: string;
          };
        } else {
          result = (await editProductClass(localParams.value)) as unknown as {
            status: string;
            message: string;
          };
        }
