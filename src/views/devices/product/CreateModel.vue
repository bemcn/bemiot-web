  ...
  >
    ...
    >
      <div class="from-group">
        ...
        <n-form-item label="所属分组" path="modelType">
          <n-select
            v-model:value="localParams.modelType"
        ...
        <n-form-item label="模型类别">
          <n-button-group>
            <n-button
              :type="localParams.modelClass == 1 ? 'info' : 'default'"
              style="width: 100px; border-radius: 4px 0px 0px 4px"
              @click="handleModelClass(1)"
              >属性</n-button
            >
            <n-button
              :type="localParams.modelClass == 2 ? 'info' : 'default'"
              style="width: 100px"
              @click="handleModelClass(2)"
              >功能</n-button
            >
            <n-button
              :type="localParams.modelClass == 3 ? 'info' : 'default'"
              style="width: 100px; border-radius: 0px 4px 4px 0px"
              @click="handleModelClass(3)"
              >事件</n-button
            >
          </n-button-group>
        </n-form-item>
        <n-form-item label="模型特性">
          ...
          >
          <n-checkbox
            :checked="(localParams.modelClass === null ? 1 : localParams.modelClass) === 1"
            @update:checked="handleMonitor"
            style="margin-right: 15px"
            v-if="(localParams.modelClass === null ? 1 : localParams.modelClass) === 1"
            >实时监测</n-checkbox
          >
          <n-checkbox
      ...
      <div class="from-group">
        ...
        <n-form-item label="最大长度" v-if="showState && localParams.dataType === 'text'">
          <n-input
            :maxlength="3"
            placeholder="请输入最大长度255以内，例如: 255"
        ...
        <n-form-item label="枚举项" v-if="showState && localParams.dataType === 'enum'">
          <div class="enum-params">
            <div class="enum-line" v-for="(item, index) in localParams.datas.values" :key="index">
              ...
              <div class="enum-delete" v-if="Number(index) > 0">
                <a href="javascript:;" @click="deleteEnumItem(Number(index))">
                  <n-icon size="18">
                    <CloseCircleFilled />
                  </n-icon>
                </a>
...
<script lang="ts" setup>
  ...
  import { addProductModels } from '@/api/devices/productModel';
  // @ts-ignore
  ...
  // 类别切换
  const handleModelClass = (value: number) => {
    if (value === 1) {
    ...
    localParams.value.modelClass = value;
  };
  // 模型特性选择
  ...
  const deleteEnumItem = (index: number) => {
    localParams.value.datas.values.splice(index, 1);
  };
  ...
  const confirmForm = (e: any) => {
    ...
    formRef.value.validate(async (errors: any) => {
      if (!errors) {
        if (localParams.value.dataType === 'text') {
          const maxLengthStr = localParams.value.datas.maxLength;
          if (maxLengthStr === '' || maxLengthStr === '0') {
            localParams.value.datas.maxLength = '255';
          } else {
        ...
        const params = {
          modelId: localParams.value.modelId,
          modelIdentity: localParams.value.modelIdentity,
          modelName: localParams.value.modelName,
          productId: localParams.value.productId,
          modelType: parseInt(localParams.value.modelType),
          modelClass: localParams.value.modelClass,
          dataType: localParams.value.dataType,
          dataDefinition: encodeURI(JSON.stringify(localParams.value.datas)),
        ...
        let result = (await addProductModels(params)) as unknown as {
          status: string;
          message: string;
        };
