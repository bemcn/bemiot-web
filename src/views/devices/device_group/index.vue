...
<script lang="ts" setup>
  ...
  import { DeviceGroup, PageDeviceGroup } from '@/types/BaseModel';
  import {
    getDeviceGroupPageList,
    updateDeviceGroupOrder,
    delDeviceGroupById,
    delDeviceGroupByIds,
  } from '@/api/base/deviceGroup';
  ...
  const loadDataTable = async (res: any) => {
    ...
    const result = (await getDeviceGroupPageList(params)) as unknown as {
      status: string;
      message: string;
      data: PageDeviceGroup;
    };
  ...
  const handleDel = async (record: Recordable) => {
    ...
    try {
      const result = (await delDeviceGroupById(params)) as unknown as {
        status: string;
        message: string;
      };
  ...
  const handleDelArray = async () => {
    ...
    const result = (await delDeviceGroupByIds(params)) as unknown as {
      status: string;
      message: string;
    };
    if (result.status === 'success') {
  ...
  // 更新排序值的函数
  const editOrderNumber = async (index: number, value: number, record: DeviceGroup) => {
    try {
      // 发送请求更新后端数据
      const params = {
      ...
      const result = (await updateDeviceGroupOrder(params)) as unknown as {
        status: string;
        message: string;
      };
