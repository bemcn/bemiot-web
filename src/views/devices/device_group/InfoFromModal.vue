...
<script lang="ts" setup>
  ...
  import { addDeviceGroup, editDeviceGroup } from '@/api/base/deviceGroup';
  ...
  const confirmForm = (e: any) => {
    ...
    formRef.value.validate(async (errors: any) => {
      if (!errors) {
        ...
        if (props.action === 'add') {
          result = (await addDeviceGroup(localParams.value)) as unknown as {
            status: string;
            message: string;
          };
        } else {
          result = (await editDeviceGroup(localParams.value)) as unknown as {
            status: string;
            message: string;
          };
        }
