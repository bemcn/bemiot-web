...
<script lang="ts" setup>
  ...
  const handleEdit = (record: Recordable) => {
    ...
    action.value = 'editScene';
    viewMain.value = false;
  };

  const saveRulesEditor = async (data: any) => {
    console.log('data', data);
    const params = {
      sceneId: data.sceneId,
      sceneGroupId: data.sceneGroupId,
      sceneName: formParams.value.sceneName,
      executionMethod: formParams.value.executionMethod,
      silentPeriod: formParams.value.silentPeriod,
      timeConditions: formParams.value.timeConditions,
      elData: JSON.stringify(data),
      remark: formParams.value.remark,
    }

    // const result = (await saveSceneLinkage(formParams.value)) as unknown as {
    //   status: string;
    //   message: string;
    // };
    viewMain.value = true;
