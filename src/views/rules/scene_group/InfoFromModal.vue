...
<script lang="ts" setup>
  ...
  import { addSceneGroup, editSceneGroup } from '@/api/rules/sceneGroup';
  ...
  const confirmForm = (e: any) => {
    ...
    formRef.value.validate(async (errors: any) => {
      if (!errors) {
        ...
        if (props.action === 'add') {
          result = (await addSceneGroup(localParams.value)) as unknown as {
            status: string;
            message: string;
          };
        } else {
          result = (await editSceneGroup(localParams.value)) as unknown as {
            status: string;
            message: string;
          };
        }
