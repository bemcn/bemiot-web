...
<script lang="ts" setup>
  ...
  import { SceneGroup, PageSceneGroup } from '@/types/RulesModel';
  import {
    getSceneGroupPageList,
    updateSceneGroupOrder,
    delSceneGroupById,
    delSceneGroupByIds,
  } from '@/api/rules/sceneGroup';
  ...
  const auth = userStore.parseAuthByModule('scene_group');
  ...
  const loadDataTable = async (res: any) => {
    ...
    const result = (await getSceneGroupPageList(params)) as unknown as {
      status: string;
      message: string;
      data: PageSceneGroup;
    };
  ...
  const handleEdit = (record: Recordable) => {
    formParams.value = {
      sceneGroupId: record.sceneGroupId,
  ...
  const handleDel = async (record: Recordable) => {
    const params = {
      id: record.sceneGroupId,
    };

    try {
      const result = (await delSceneGroupById(params)) as unknown as {
        status: string;
        message: string;
      };
  ...
  const handleDelArray = async () => {
    ...
    const result = (await delSceneGroupByIds(params)) as unknown as {
      status: string;
      message: string;
    };
    if (result.status === 'success') {
  ...
  // 更新排序值的函数
  const editOrderNumber = async (index: number, value: number, record: SceneGroup) => {
    try {
      // 发送请求更新后端数据
      const params = {
        id: Number(record.sceneGroupId), // 强制转换为 number 类型
        orderNum: value,
      };

      const result = (await updateSceneGroupOrder(params)) as unknown as {
        status: string;
        message: string;
      };
