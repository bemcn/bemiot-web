        ...
        if (inputValue !== '') {
          try {
            if (inputType === 'date' || inputType === 'datetime') {
              ...
              const timestamp = date.getTime() + 86400000;

              const optionDate = new Date(timestamp);
              if (inputType === 'date') {
                outputValue = format(optionDate, 'yyyy-MM-dd');
              } else {
                outputValue = format(optionDate, 'yyyy-MM-dd HH:mm:ss');
              }
            } else {
              let timestamp = 0;
              if (inputValue.length === 10) {
                timestamp = Number(inputValue) + 86400;
              } else {
                timestamp = Number(inputValue) + 86400000;
              }
