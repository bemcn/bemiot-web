...
<script lang="ts" setup>
  ...
  const handleFunc = (_value: string, option: SelectBaseOption | null) => {
    const inputIdNum = localParams.value.inputIdNum;
    ...
    if (inputNodes !== null && inputNodes.length > 0) {
      ...
      const inputIdType2 = option?.inputIdType2;
      for (let i = 0; i < inputNodes; i++) {
        ...
        if (logicFlowUtil.logicFlow) {
          ...
          if (pops) {
            ...
            if (pops.valueType) {
              if (inputIdType1 === 'number|int') {
                if (pops.valueType === 'number' || pops.valueType === 'int') {
                  idArray1.push(item);
                }
              } else {
                if (pops.valueType === inputIdType1) {
                  idArray1.push(item);
                }
              }
              if (inputIdNum === 2) {
                if (inputIdType2 === 'number|int') {
                  if (pops.valueType === 'number' || pops.valueType === 'int') {
                    idArray2.push(item);
                  }
                } else {
                  if (pops.valueType === inputIdType2) {
                    idArray2.push(item);
                  }
                }
              }
