<template>
  ...
  >
    <n-form
      ...
      class="py-4"
    >
      <n-form-item :span="8" label="场景名称" path="sceneName">
        <n-input
          maxlength="50"
          placeholder="请输入场景名称"
      ...
      <n-form-item :span="8" label="所属分组" path="sceneGroupId">
        <n-select
      ...
      <n-form-item :span="8" label="执行方式" path="executionMethod">
        <n-select
              ...
              <div class="rule-value">
                <n-checkbox-group v-model:value="localParams.timeConditionsObj.repeatMonths">
                  <n-space item-style="display: flex;">
                    <n-grid x-gap="20" y-gap="10" :cols="8">
                      <n-gi>
      ...
      <n-form-item :span="24" label="备注" path="remark">
        <n-input
          type="textarea"
          placeholder="请输入备注"
          :autosize="{
...
<script lang="ts" setup>
  ...
  import { SceneGroup } from '@/types/RulesModel';
  import { addSceneLinkage, editSceneLinkage } from '@/api/rules/sceneLinkage';
  // @ts-ignore
  ...
  const confirmForm = (e: any) => {
    ...
    formRef.value.validate(async (errors: any) => {
      if (!errors) {
        const timeConditionsObj = localParams.value.timeConditionsObj;
        const timeConditions = JSON.stringify(timeConditionsObj);
        const params = {
          sceneId: localParams.value.sceneId,
          sceneGroupId: localParams.value.sceneGroupId,
          sceneName: localParams.value.sceneName,
          executionMethod: localParams.value.executionMethod,
          silentPeriod: localParams.value.silentPeriod,
          timeConditions,
          elData: localParams.value.elData,
          remark: localParams.value.remark,
        };
        console.log('params', params);

        let result: { status: any; message: any };
        if (props.action === 'addScene') {
          result = (await addSceneLinkage(params)) as unknown as {
            status: string;
            message: string;
          };
        } else {
          result = (await editSceneLinkage(params)) as unknown as {
            status: string;
            message: string;
          };
        }
