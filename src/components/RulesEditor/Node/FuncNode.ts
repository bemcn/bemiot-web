...
class FuncView extends HtmlNode {
  ...
  setHtml(rootEl: SVGForeignObjectElement) {
    ...
    el.className = 'node-wrapper func-wrapper';
    el.style.height = height + 'px'; // 设置DOM元素高度
    el.style.width = width + 'px';

    let formula = '函数公式';
    let describe = '函数方法';
    if (properties.type === 'format') {
      formula = '转换公式';
      describe = '转换方法';
    }
    let inputVal = '';
    if (Array.isArray(properties.inputValue) && properties.inputValue.length > 0) {
      inputVal = properties.inputValue.join(',');
    }

    const html = `
      <div class="nodeBox">
        <div class="node-head func-head">
          <div class="tag func-tag">${properties.label}</div>
        ...
        <div class="node-body">
          <div class="node-content">
            <div class="line">
              ...
              <div class="value">${properties.describe || ''}</div>
            </div>
            <div class="line">
              <div class="label">${formula}</div>
              <div class="value">${properties.formula || ''}</div>
            </div>
          </div>
        </div>
        <div class="node-footer func-footer">
          <div class="foot-line">
...
class FuncModel extends HtmlNodeModel {
  setAttributes() {
