...
class BridgingView extends HtmlNode {
  ...
  setHtml(rootEl: SVGForeignObjectElement) {
    ...
    el.className = 'node-wrapper execute-wrapper';
    ...
    if (properties.type === 'dataStorage') {
      tagName = '数据存储';
      fromName = '存储表名';
      fromValue = (properties.dataName as string) || '';
    } else if (properties.type === 'messagePush') {
      tagName = '消息推送';
      fromName = '消息地址';
      fromValue = (properties.mqttUrl as string) || '';
    } else {
      tagName = '数据推送';
      fromName = '推送地址';
      fromValue = (properties.httpUrl as string) || '';
    }

    let html = `
      <div class="nodeBox">
        <div class="node-head execute-head">
          <div class="tag execute-tag">${tagName}</div>
        ...
        <div class="node-body" style="padding-bottom: 0px;">
          <div class="node-content">
            <div class="line">
              <div class="label">桥接名称</div>
              <div class="value">${properties.bridgeName || ''}</div>
            </div>
            <div class="line">
              <div class="label">${fromName}</div>
              <div class="value">${fromValue}</div>
            </div>
    `;
    if (properties.filedArray) {
      for (const item of properties.filedArray as any[]) {
        html += `
            <div class="line">
              <div class="label">${item.filed || ''}</div>
              <div class="value">${item.filedName || ''}</div>
            </div>
        `;
      }
    }
...
class BridgingModel extends HtmlNodeModel {
  setAttributes() {
    this.width = 240;
    this.height = 108;
