...
class BridgingView extends HtmlNode {
  ...
  setHtml(rootEl: SVGForeignObjectElement) {
    ...
    let fileds = '';
    let values = '';
    if (Array.isArray(properties.fileds) && properties.fileds.length > 0) {
      for (let i = 0; i < properties.fileds.length; i++) {
        if (i === 0) {
          fileds = properties.fileds[i].filedName;
        } else {
          fileds += ',' + properties.fileds[i].filedName;
        }
      }
    }
    if (Array.isArray(properties.values) && properties.values.length > 0) {
      for (let i = 0; i < properties.values.length; i++) {
        if (i === 0) {
          values = properties.values[i].valueName;
        } else {
          values += ',' + properties.values[i].valueName;
        }
      }
      ...
      <div class="nodeBox">
        ...
        <div class="node-body" style="padding-bottom: 0px;">
          <div class="node-content">
            <div class="line">
              <div class="label">存储表名</div>
              <div class="value">${properties.tableName || ''}</div>
            </div>
            <div class="line">
              <div class="label">存储字段</div>
              <div class="value">${fileds}</div>
            </div>
            <div class="line">
