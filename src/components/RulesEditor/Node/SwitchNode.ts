...
class SwitchView extends HtmlNode {
  ...
  setHtml(rootEl: SVGForeignObjectElement) {
    ...
    if (
      ...
      count = properties.condition.length;
    }
    if (count > 0) {
      count++;
    }
    let nodeId = '--';
    const inputNodes: string[] | null = properties.inputNodes as string[] | null;
    ...
    const html = `
      <div class="nodeBox">
        <div class="node-head judgment-head">
          <div class="tag judgment-tag">分支</div>
        ...
        <div class="node-body" style="padding-bottom: 0px;">
          <div class="node-content">
            <div class="ifLine">
              <div class="swLabel">
                <div class="term">SWITCH</div>
              </div>
              <div class="csLabel">
                <div class="term">${count}</div>
              </div>
              <div class="swValue">${nodeId}</div>
            </div>
          </div>
        </div>
      </div>
...
class SwitchModel extends HtmlNodeModel {
  setAttributes() {
  ...
  isAllowConnectedAsSource(target, targetAnchor, _sourceAnchor) {
    ...
    if (index === 1) {
      ...
      if (target) {
        ...
        const conditionNum = currentSourceProperties?.condition.length;
        if (conditionNum > 0) {
          let linkNum: number;
          if (currentSourceProperties?.outputNodes === null) {
            linkNum = 0;
          } else {
            linkNum = currentSourceProperties?.outputNodes.length;
          }
          if (linkNum > conditionNum) {
            const maxLen = conditionNum + 1;
            return {
              isAllPass: false,
              msg: 'msg:右侧锚点连线已超过判断数量，最大数量' + maxLen + '条',
            };
          }
        } else {
          return {
            isAllPass: false,
            msg: 'msg:请先设置switch判断条件',
          };
        }

        if (sourceNode && currentSourceProperties && targetNode) {
          ...
          const conditionArray = currentSourceProperties.condition;
          let hasCase = false;
          for (let i = 0; i < conditionArray.length; i++) {
            ...
            if (toNodeId === '') {
              hasCase = true;
              conditionArray[i].toNodeId = targetNode.id;
              break;
            }
          }
  ...
  // 更新节点高度的方法
  setTargetIds(number: number): void {
    const n = number + 2;
    const ids: string[] = [];
    const size = this.targetIds.length;
    for (let i = 0; i < n; i++) {
      if (i < size) {
        ids.push(this.targetIds[i]);
      } else {
        ids.push('');
      }
    }
