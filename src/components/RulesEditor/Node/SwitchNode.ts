...
class SwitchView extends HtmlNode {
      ...
      <div class="nodeBox">
        ...
        <div class="node-body" style="padding-bottom: 0px;">
          <div class="node-content">
            <div class="jgLine">
              <div class="label">${properties.nodeId || '--'}</div>
            </div>
    `;
    // 修复类型错误：添加类型检查
    if (
  ...
  private updateNodeDimensions(model: any, newWidth: number, newHeight: number) {
    ...
    // 锚点的更新坐标
    const condition = currentProperties.condition;
    let count: number;
...
class SwitchModel extends HtmlNodeModel {
  setAttributes() {
    ...
    const heightAnc = xHeight + 36 + 6 + 18;
    const heightAnc1 = heightAnc + 32;
    const heightRightFootAnc = heightAnc1 + 32;
    this.anchorsOffset = [
      [-widthAnc, heightAnc], // 左侧输入
      [widthAnc, heightAnc1], // 右侧case
      [widthAnc, heightRightFootAnc], // 右侧default
    ];
  }
