...
class VariableView extends HtmlNode {
  ...
  setHtml(rootEl: SVGForeignObjectElement) {
    ...
    el.className = 'node-wrapper variable-wrapper';
    el.style.height = height + 'px'; // 设置DOM元素高度
    el.style.width = width + 'px';

    const html = `
      <div class="nodeBox">
        <div class="node-head variable-head">
          <div class="tag variable-tag">${properties.label}</div>
...
class VariableModel extends HtmlNodeModel {
  setAttributes() {
    const properties = this.getProperties();
    this.width = properties.width || 200;
    this.height = properties.height || 132;
  ...
  getAnchorStyle(anchorInfo: LogicFlow.Point | undefined) {
      ...
      if (target) {
        ...
        if (allowedTypes.includes(type)) {
          ...
          if (type === 'attribute' || type === 'variable') {
            ...
            if (
              sourceValueType !== 'time' &&
              sourceValueType !== 'date' &&
              sourceValueType !== 'datetime' &&
              sourceValueType !== 'timestamp'
            ) {
              return {
                isAllPass: false,
                msg: 'msg:当前节点非日期类型',
