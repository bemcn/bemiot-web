...
class ParallelView extends HtmlNode {
  ...
  setHtml(rootEl: SVGForeignObjectElement) {
    ...
    el.className = 'node-wrapper parallel-wrapper';
    el.style.height = height + 'px'; // 设置DOM元素高度
    el.style.width = width + 'px';

    let modes = '实时执行';
    const inputNodes = properties.inputNodes as string[] | null;
    ...
    const html = `
      <div class="nodeBox">
        <div class="node-head parallel-head">
          <div class="tag parallel-tag">${properties.label}</div>
        ...
        <div class="node-body" style="padding-bottom: 0px;">
          <div class="node-content">
            <div class="line" style="height: 38px;">
              <div class="label">执行模式</div>
              <div class="values">${modes}</div>
            </div>
          </div>
        </div>
      </div>
...
class ParallelModel extends HtmlNodeModel {
  setAttributes() {
    this.width = 200;
    this.height = 74;
