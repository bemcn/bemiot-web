...
class RandomView extends HtmlNode {
  ...
  setHtml(rootEl: SVGForeignObjectElement) {
    ...
    el.className = 'node-wrapper random-wrapper';
    el.style.height = height + 'px'; // 设置DOM元素高度
    el.style.width = width + 'px';

    const html = `
      <div class="nodeBox">
        <div class="node-head random-head">
          <div class="tag random-tag">${properties.label}</div>
        ...
        <div class="node-body">
          <div class="node-content">
            <div class="line">
              ...
              <div class="value">
              (${properties.valueType || 'int'}) 
              ${properties.minNum || 0}-${properties.maxNum || 10}
              </div>
            </div>
            <div class="line">
              <div class="label">小数位</div>
              <div class="value">${
                ((properties.bits as number) ?? 0) > 0 ? properties.bits + '位' : '无'
              }</div>
            </div>
            <div class="line">
              <div class="label">刷新间隔</div>
              <div class="value">${properties.refresh || 10}秒</div>
            </div>
          </div>
        </div>
        <div class="node-footer random-footer">
          <div class="foot-line" style="border: 0px">
            <div class="label">输出值</div>
            <div class="refresh">
              <button class="btn-edit" title="刷新" onclick="handleRefRandom('${
                this.props.model.id
              }')" />
            </div>
...
class RandomModel extends HtmlNodeModel {
  setAttributes() {
    const properties = this.getProperties();
    this.width = properties.width || 240;
    this.height = properties.height || 150;
  ...
  // 允许作为目标节点（允许连入）
  isAllowConnectedAsTarget(_source, _sourceAnchor, _targetAnchor) {
    return {
      isAllPass: false,
      msg: 'msg:禁止逆向连接',
    };
  ...
  isAllowConnectedAsSource(target, _targetAnchor, _sourceAnchor) {
