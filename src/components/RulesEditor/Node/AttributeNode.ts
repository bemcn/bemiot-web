...
class AttributeView extends HtmlNode {
  ...
  setHtml(rootEl: SVGForeignObjectElement) {
    ...
    el.className = 'node-wrapper object-wrapper';
    el.style.height = height + 'px'; // 设置DOM元素高度
    el.style.width = width + 'px';

    let abnormal = 0;
    let abnormalMsg = '';
    if (properties.id === null || properties.id === '') {
      abnormal = 1;
      abnormalMsg = '未选择数据对象';
    } else {
      if (properties.outputValue === '' || properties.outputValue === 'none') {
        abnormal = 2;
        abnormalMsg = '没有数据输出';
      }
    }
    ...
    let html = `
      <div class="nodeBox">
        <div class="node-head object-head">
          <div class="tag object-tag">数据对象</div>
          <div class="label">${this.props.model.id || ''}</div>
          <div class="editBox">
    `;
    if (properties.inputValue === null || properties.inputValue === '') {
      html += `
            <button class="btn-edit" title="编辑数据对象" onclick="handleEditorSubmit('${this.props.model.id}')" />
      `;
    }
    html += `
            <button class="btn-close" title="删除" onclick="handleNodeClose('${this.props.model.id}')" />
          </div>
        </div>
        <div class="node-body">
    ...
    if (abnormal === 1) {
      html += `
        <div class="alarmIco" title="${abnormalMsg}"></div>
      `;
    } else {
      html += `
    ...
    html += `
        ...
        <div class="node-footer object-footer">
          <div class="foot-line">
            ...
            <div class="value">${properties.valueType || ''}</div>
          </div>
          <div class="foot-line">
    ...
    if (abnormal === 2) {
      ...
      html += `
        <div class="valueDoc">${properties.outputValue || ''}</div>
      `;
    }
    html += `
            </div>
...
class AttributeModel extends HtmlNodeModel {
  setAttributes() {
    const properties = this.getProperties();
    this.width = properties.width || 240;
    this.height = properties.height || 220;
        ...
        if (allowedTypes.includes(type)) {
          ...
          if (sourceNode && targetNode) {
            ...
            if (type === 'attribute' || type === 'variable') {
              ...
              inputValue = sourceProperties?.outputValue || sourceProperties?.defaultValue;
              const inputType = sourceProperties?.valueType;
