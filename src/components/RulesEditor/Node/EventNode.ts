...
class EventView extends HtmlNode {
  ...
  setHtml(rootEl: SVGForeignObjectElement) {
    ...
    el.className = 'node-wrapper event-wrapper';
    el.style.height = height + 'px'; // 设置DOM元素高度
    el.style.width = width + 'px';

    const html = `
      <div class="nodeBox">
        <div class="node-head event-head">
...
class EventModel extends HtmlNodeModel {
  setAttributes() {
  ...
  isAllowConnectedAsSource(target, _targetAnchor, _sourceAnchor) {
    ...
    if (target) {
      ...
      // 允许连接的类型列表
      const allowedTypes = ['judgment', 'switch', 'parallel', 'export', 'export'];
