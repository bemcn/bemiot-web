    ...
    <template v-if="showLogin">
      <div class="login-box">
        ...
        <n-input
          type="password"
          autofocus
          v-model:value="loginParams.userPwd"
          @keyup.enter="onLogin"
          placeholder="请输入登录密码"
        >
          <template #suffix>
...
<script lang="ts">
  ...
  import { AUTO_LOGIN } from '@/store/mutation-types';
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      // 登录
      const onLogin = async () => {
        if (!state.loginParams.userPwd.trim()) {
          return;
        }
        ...
        if (resultCode.status === 'success') {
          ...
          const params = {
            ...
            userPwd: state.loginParams.userPwd,
            code,
          };
          const autoLogin = state.loginParams.autoLogin;
          state.loginLoading = true;

          const result = await userStore.login(params, autoLogin);
          ...
          state.errorMsg = resultCode.message;
          state.isLoginError = true;
        }
