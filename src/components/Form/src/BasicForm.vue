  ...
  <n-form v-bind="getBindValue" :model="formModel" ref="formElRef">
    <n-grid v-bind="getGrid">
      <n-gi v-bind="schema.giProps" v-for="schema in getSchema" :key="schema.field">
        <n-form-item label-width="'auto'" :label="schema.label" :path="schema.field">
          <!--标签名右侧温馨提示-->
          ...
          <template v-else-if="schema.component === 'NCheckbox'">
            <n-checkbox-group v-model:value="formModel[schema.field]" style="margin-right: 25px">
              <n-space>
                <n-checkbox
          ...
          <template v-else-if="schema.component === 'NRadioGroup'">
            <n-radio-group v-model:value="formModel[schema.field]" style="margin-right: 25px">
              <n-space>
                <n-radio
                  v-for="item in schema.componentProps.options"
                  :key="item.value"
          ...
          <template v-else-if="schema.component === 'NInputGroup'">
            <n-input-group style="margin-right: 25px">
              <n-select
                :style="schema.subFormSchema?.style"
                v-model:value="formModel[schema.subFormSchema?.field ?? '']"
                :options="schema.subFormSchema?.componentProps.options ?? []"
              />
              <n-input v-model:value="formModel[schema.field]" maxlength="50" />
            </n-input-group>
          </template>
