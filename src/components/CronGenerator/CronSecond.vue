<template>
  <n-radio-group v-model:value="modelValue" name="radiogroup">
    <n-space vertical>
      ...
      <n-space align="center">
        <n-radio value="specific">具体秒数(可多选)</n-radio>
        <n-select
          v-model:value="specificSeconds"
          multiple
...
<script lang="ts" setup>
  ...
  const specificSeconds = ref<number[]>([0]);
  ...
  watch([interval, specificSeconds, rangeStart, rangeEnd], () => {
    updateValue();
  });
  ...
  function updateValue() {
    ...
    switch (modelValue.value) {
      ...
      case 'custom':
        ...
        break;
      case 'specific':
        value = specificSeconds.value.join(',');
        break;
      case 'range':
  ...
  watch(
    ...
    (val) => {
      if (val === '0') {
        ...
        specificSeconds.value = val.split(',').map(Number);
      } else if (val.includes('-')) {
